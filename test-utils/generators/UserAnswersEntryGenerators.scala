...
trait UserAnswersEntryGenerators extends PageGenerators with ModelGenerators {
  ...
  implicit lazy val arbitraryAmountOfNewTaxChargeYouPayUserAnswersEntry: Arbitrary[(AmountOfNewTaxChargeYouPayPage.type, JsValue)] =
    Arbitrary {
      for {
        page  <- arbitrary[AmountOfNewTaxChargeYouPayPage.type]
        value <- arbitrary[Int].map(Json.toJson(_))
      } yield (page, value)
    }

  implicit lazy val arbitraryAmountOfNewTaxChargeSchemePayUserAnswersEntry: Arbitrary[(AmountOfNewTaxChargeSchemePayPage.type, JsValue)] =
    Arbitrary {
      for {
        page  <- arbitrary[AmountOfNewTaxChargeSchemePayPage.type]
        value <- arbitrary[Int].map(Json.toJson(_))
      } yield (page, value)
    }
  ...
  implicit lazy val arbitraryPensionSchemeMemberInternationalAddressUserAnswersEntry
    ...
    Arbitrary {
      for {
        ...
        value <- arbitrary[PensionSchemeMemberInternationalAddress].map(Json.toJson(_))
      } yield (page, value)
    }
  ...
  implicit lazy val arbitraryPensionSchemeMemberUKAddressUserAnswersEntry
    ...
    Arbitrary {
      for {
        ...
        value <- arbitrary[PensionSchemeMemberUKAddress].map(Json.toJson(_))
      } yield (page, value)
    }
