...
class BavfConfig @Inject() (
  ...
  val retrieveURL: String = s"$bavfApiBaseUrl/api/v3/complete"
  ...
  def initRequestConfig(continueUrl: String, bankDetailsAnswers: Option[BankDetails])() = {
    ...
    val barsRequestJson =
      s"""{
         ...
         |    "name": "${bankDetailsAnswers.map(details => details.accountName).getOrElse("")}",
         |    "sortCode": "${bankDetailsAnswers.map(details => details.sortCode).getOrElse("")}",
         |    "accountNumber": "${bankDetailsAnswers.map(details => details.accountNumber).getOrElse("")}",
         |    "rollNumber": "${bankDetailsAnswers.flatMap(details => details.rollNumber).getOrElse("")}"
         |  },
