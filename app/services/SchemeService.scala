...
object SchemeService {
  ...
  private def schemesFromAAInputs(calculationInputs: CalculationInputs) =
    calculationInputs.annualAllowance
      .map {
        _.taxYears flatMap {
          case TaxYear2016To2023.NormalTaxYear(_, taxYearSchemes, _, _, _, _, _, _)                                 =>
            List(taxYearSchemes)
          case TaxYear2016To2023.InitialFlexiblyAccessedTaxYear(_, _, _, _, taxYearSchemes, _, _, _, _, _, _, _, _) =>
            List(taxYearSchemes)
          case TaxYear2016To2023.PostFlexiblyAccessedTaxYear(_, _, _, _, taxYearSchemes, _, _, _, _, _)             =>
            List(taxYearSchemes)
          case _                                                                                                    => Nil
        }
      }
