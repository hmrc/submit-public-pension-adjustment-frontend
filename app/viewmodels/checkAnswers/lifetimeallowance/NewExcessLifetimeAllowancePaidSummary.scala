...
object NewExcessLifetimeAllowancePaidSummary {
  ...
  def row(submission: Submission)(implicit messages: Messages): Option[SummaryListRow] =
    submission.calculationInputs.lifeTimeAllowance.flatMap { lta =>
      lta.newLifeTimeAllowanceAdditions.newExcessLifetimeAllowancePaid.map { newExcessLifetimeAllowancePaid =>
        val value = ValueViewModel(
          HtmlContent(HtmlFormat.escape(messages(s"newExcessLifetimeAllowancePaid.$newExcessLifetimeAllowancePaid")))
        )
