...
object QuarterChargePaidSummary {
  ...
  def row(submission: Submission)(implicit messages: Messages): Option[SummaryListRow] =
    submission.calculationInputs.lifeTimeAllowance.flatMap { lta =>
      lta.newLifeTimeAllowanceAdditions.quarterChargePaid.map { quarterCharge =>
        val value = ValueViewModel(HtmlContent(HtmlFormat.escape(messages(s"quarterChargePaid.$quarterCharge"))))
