...
object ProtectionEnhancedChangedSummary {
  ...
  def row(submission: Submission)(implicit messages: Messages): Option[SummaryListRow] =
    submission.calculationInputs.lifeTimeAllowance.map { lta =>
      val value = ValueViewModel(
        HtmlContent(HtmlFormat.escape(messages(s"protectionEnhancedChanged.${lta.protectionTypeEnhancementChanged}")))
      )
