...
object NewAnnualPaymentValueSummary {
  ...
  def row(submission: Submission)(implicit messages: Messages): Option[SummaryListRow] =
    submission.calculationInputs.lifeTimeAllowance.flatMap { lta =>
      lta.newLifeTimeAllowanceAdditions.newAnnualPaymentValue.map { newAnnualPaymentValue =>
        SummaryListRowViewModel(
          key = KeyViewModel(s"newAnnualPaymentValue.checkYourAnswersLabel").withCssClass(keyCssClass),
          value = ValueViewModel(HtmlContent(currencyFormat(newAnnualPaymentValue)))
        )
      }
    }
}
