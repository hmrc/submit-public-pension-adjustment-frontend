...
object NewEnhancementTypeSummary {
  ...
  def row(submission: Submission)(implicit messages: Messages): Option[SummaryListRow] =
    submission.calculationInputs.lifeTimeAllowance.flatMap { lta =>
      lta.newLifeTimeAllowanceAdditions.newEnhancementType.map { newEnhancementType =>
        val value = ValueViewModel(
          HtmlContent(HtmlFormat.escape(messages(s"newEnhancementType.$newEnhancementType")))
        )
