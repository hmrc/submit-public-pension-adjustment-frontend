...
object UserSchemeDetailsSummary {
  ...
  def row(submission: Submission)(implicit messages: Messages): Option[SummaryListRow] =
    submission.calculationInputs.lifeTimeAllowance.flatMap { lta =>
      lta.newLifeTimeAllowanceAdditions.userSchemeDetails.map { userSchemeDetails =>
        val value = HtmlFormat.escape(userSchemeDetails.name).toString + " / " + HtmlFormat
          .escape(userSchemeDetails.taxRef)
          .toString
