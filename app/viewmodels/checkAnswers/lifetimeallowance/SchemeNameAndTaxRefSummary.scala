...
object SchemeNameAndTaxRefSummary {
  ...
  def row(submission: Submission)(implicit messages: Messages): Option[SummaryListRow] =
    submission.calculationInputs.lifeTimeAllowance.flatMap { lta =>
      lta.previousLifetimeAllowanceChargeSchemeNameAndTaxRef.map { schemeDetails =>
        val value =
          HtmlFormat.escape(schemeDetails.name).toString + " / " + HtmlFormat.escape(schemeDetails.taxRef).toString
