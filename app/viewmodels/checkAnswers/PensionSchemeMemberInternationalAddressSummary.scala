...
object PensionSchemeMemberInternationalAddressSummary {

  def row(answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(PensionSchemeMemberInternationalAddressPage).map { answer =>
      val value = Seq(
      ...
      SummaryListRowViewModel(
        ...
        actions = Seq(
          ActionItemViewModel(
            "site.change",
            //TODO change
            routes.JourneyRecoveryController.onPageLoad(None).url
          )
