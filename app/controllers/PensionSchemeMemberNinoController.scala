...
class PensionSchemeMemberNinoController @Inject() (
  ...
  formProvider: PensionSchemeMemberNinoFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: PensionSchemeMemberNinoView
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] =
    (identify andThen getData andThen requireCalculationData andThen requireData) { implicit request =>
      val preparedForm = request.userAnswers.get(PensionSchemeMemberNinoPage) match {
        case None        => form
