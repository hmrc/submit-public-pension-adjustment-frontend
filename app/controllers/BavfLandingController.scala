...
class BavfLandingController @Inject() (
  ...
  def redirectBars(id: String, mode: Mode): Action[AnyContent] = (identify
    andThen getData
    andThen requireCalculationData
    andThen requireData).async { implicit request =>
    barsHandlerFactory(id, request, mode)
  }

  private def barsHandlerFactory(id: String, request: DataRequest[AnyContent], mode: Mode)(implicit
    hc: HeaderCarrier
  ) =
    for {
      retrieveAddress <- bavfConnector.complete(id)
      updatedAnswers  <- barsParser(request, retrieveAddress)
      redirectUrl      = BankDetailsNavigation.navigate(updatedAnswers, request.submission, mode)
      answersWithNav   = NavigationState.save(updatedAnswers, redirectUrl.url)
      _               <- userDataService.set(answersWithNav)
    } yield Redirect(redirectUrl)

  private def barsParser(
