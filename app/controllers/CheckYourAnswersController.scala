...
import pages.{ClaimOnBehalfPage, PageValidation}
...
class CheckYourAnswersController @Inject() (
  ...
  def onPageLoad(): Action[AnyContent] = (identify andThen getData andThen requireCalculationData andThen requireData) {
    implicit request =>
      val incompleteRequiredPages = PageValidation.incompleteRequiredPages(request.submission, request.userAnswers)
      logger.warn(s"incompleteRequiredPages : $incompleteRequiredPages")
