...
class PensionSchemeMemberResidenceController @Inject() (
  ...
  formProvider: PensionSchemeMemberResidenceFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: PensionSchemeMemberResidenceView
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] =
    (identify andThen getData andThen requireCalculationData andThen requireData) { implicit request =>
      val preparedForm = request.userAnswers.get(PensionSchemeMemberResidencePage) match {
        case None        => form
      ...
      form
        ...
        .fold(
          ...
          value =>
            ...
            } yield Redirect(PensionSchemeMemberResidencePage.navigate(mode, updatedAnswers))
        )
    }
}
