...
class AddressLookupConnectorSpec extends SpecBase with WireMockHelper with Matchers {
  ...
  lazy val requestALF =
    readRequestData("test/resources/ClaimOnBehalfALFRequest.json")
  ...
  "initialiseJourney" - {
    ...
    "must return a result when the server responds with Accepted" in {
      ...
      running(app) {
        ...
        val result = connector.initialiseJourney(requestALF).futureValue
    ...
    "must error a result when the server responds with any other status" in {
      ...
      running(app) {
        ...
        val result = connector.initialiseJourney(requestALF).failed.futureValue
