...
import models.{Done, NormalMode, StatusOfUser, UserAnswers}
...
import views.html.StatusOfUserView

import scala.concurrent.Future

class StatusOfUserControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  lazy val statusOfUserRoute = routes.StatusOfUserController.onPageLoad(NormalMode).url

  lazy val calculationPrerequisiteRoute = routes.CalculationPrerequisiteController.onPageLoad().url

  val formProvider = new StatusOfUserFormProvider()
  val form         = formProvider()

  "StatusOfUser Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, statusOfUserRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[StatusOfUserView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(StatusOfUserPage, StatusOfUser.values.head).success.value
      ...
      running(application) {
        ...
        status(result) `mustEqual` OK
        contentAsString(result) `mustEqual` view(form.fill(StatusOfUser.values.head), NormalMode)(
          request,
