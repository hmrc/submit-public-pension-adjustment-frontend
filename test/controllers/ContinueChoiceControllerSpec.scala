...
class ContinueChoiceControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  lazy val continueChoiceRoute = routes.ContinueChoiceController.onPageLoad().url

  val formProvider = new ContinueChoiceFormProvider()
  val form         = formProvider()

  "ContinueChoice Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, continueChoiceRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(ContinueChoicePage, ContinueChoice.values.head).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, continueChoiceRoute)
            .withFormUrlEncodedBody(("value", ContinueChoice.values.head.toString))
