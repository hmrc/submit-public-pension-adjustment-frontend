...
import views.html.AreYouAUKResidentView

import scala.concurrent.Future

class AreYouAUKResidentControllerSpec extends SpecBase with MockitoSugar {
  ...
  "AreYouAUKResident Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, areYouAUKResidentRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[AreYouAUKResidentView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(AreYouAUKResidentPage, true).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, areYouAUKResidentRoute)
