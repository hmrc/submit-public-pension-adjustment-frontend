...
class AuthActionSpec extends SpecBase {
  ...
  "Auth Action" - {
    ...
    "the user's session has expired" - {
      ...
      "must be unauthorised if submissionUniqueId is not specified" in {
        ...
        running(application) {
          ...
          status(result) mustBe SEE_OTHER
          redirectLocation(result).value must startWith("/submit-public-pension-adjustment/unauthorised")
        }
      }
