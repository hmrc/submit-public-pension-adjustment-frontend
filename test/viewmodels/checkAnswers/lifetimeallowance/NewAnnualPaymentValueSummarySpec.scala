...
class NewAnnualPaymentValueSummarySpec extends AnyFreeSpec with Matchers {
  ...
  "row" - {
    "when value is entered, return the summary row" in {
      val newLifeTimeAllowanceAdditions = TestData.calculationInputs.lifeTimeAllowance.get.newLifeTimeAllowanceAdditions.copy(newAnnualPaymentValue = Some(999))
    ...
    "when answer unavailable, return empty" in {
      ...
      NewAnnualPaymentValueSummary.row(submission) shouldBe None
    }
  }
}
