...
class NewPensionCreditReferenceSummarySpec extends AnyFreeSpec with Matchers {
  ...
  "row" - {
    "when value is entered, return the summary row" in {
      val newLifeTimeAllowanceAdditions = TestData.calculationInputs.lifeTimeAllowance.get.newLifeTimeAllowanceAdditions.copy(newPensionCreditReference = Some("test123"))
      ...
      NewPensionCreditReferenceSummary.row(submission) shouldBe Some(
    ...
    "when answer unavailable, return empty" in {
      ...
      NewPensionCreditReferenceSummary.row(submission) shouldBe None
    }
  }
}
