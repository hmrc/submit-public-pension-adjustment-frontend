...
import models.{CheckMode, NormalMode, PensionSchemeDetails, PensionSchemeMemberUKAddress, Period, StatusOfUser, WhoWillPay}
import org.mockito.MockitoSugar.mock

import java.time.LocalDate

class PensionSchemeMemberUKAddressPageSpec extends PageBehaviours {

  "PensionSchemeMemberUKAddressPage" - {

    beRetrievable[PensionSchemeMemberUKAddress](PensionSchemeMemberUKAddressPage)

    beSettable[PensionSchemeMemberUKAddress](PensionSchemeMemberUKAddressPage)

    beRemovable[PensionSchemeMemberUKAddress](PensionSchemeMemberUKAddressPage)
  }
  ...
  "must navigate to alternative name page when LegalPersonalRepresentative answered" in {
    ...
    val page = PensionSchemeMemberUKAddressPage

    val userAnswers = emptyUserAnswers
      .set(page, arbitraryPensionSchemeMemberUKAddress.arbitrary.sample.value)
      .success
      .value
  ...
  "cleanup" - {
    "must cleanup correctly" in {
      ...
      val cleanedUserAnswers = PensionSchemeMemberUKAddressPage
        .cleanup(Some(arbitraryPensionSchemeMemberUKAddress.arbitrary.sample.value), ua)
        .success
        .value
